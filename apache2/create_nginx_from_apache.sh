...
while read VEID VEIP VESUBDIR DOMAINS ENGINE; do
    [ -z "$VEID" ] && continue
    [ "$VEID" = "#" ] && continue
    ...
    NOCACHE=$(check_if_nocache $SITE)
    if [ -n "$ENGINE" ] ; then
        echo "Disable first page cache for engine $ENGINE"
        NOCACHE=$SITE
    fi
    if [ "$ENGINE" = "mediawiki" ] ; then
        print_nginx_mediawiki_conf "$LISTENIP" $VEDIR$VESUBDIR $SITE "$OTHERDOMAINS" "$VEIP:80" "$NOCACHE" >$NGINXSITES/$SITE.conf.new
    else
